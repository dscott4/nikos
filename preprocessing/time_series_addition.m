...
[dataset] = [ones(m_old,1) dataset];
day_sequence = [1*ones(1,day_points),2*ones(1,day_points),3*ones(1,day_points),4*ones(1,day_points),5*ones(1,day_points),6*ones(1,day_points),7*ones(1,day_points),];
day_sequence = day_sequence';
day_sequence = repmat(day_sequence,[(round(m_old/(day_points*7))+1) 1]);
day_sequence = day_sequence(1:m_old,:);
dataset(:,1) = day_sequence;
year_sequence = [];
%{
for i = 1:years
